...
sys_obj_list = ('test_wake_stub.elf', 'esp_wake_stub_timer.c.obj', 'esp_wake_stub_common.c.obj', 'esp_wake_stub_ext.c.obj')


class ObjDumpError(RuntimeError):
    """Objdump runtime error class
    """
    def __init__(self, message):
        """Constructor for core dump error
        """
        super(ObjDumpError, self).__init__(message)


class SymbolInfo(object):
    """
    Encapsulates a symbol info data from objdump
    """
    ...
    def check_symb_loc(self, target):
        result = False
        symb_range = range(self.addr, self.addr + self.len)

        if target == 'esp32' or target is None:
            ...
            if self.addr in self.esp32s2_rtc_slow_mem_range or self.addr in self.esp32s2_rtc_fast_mem_range or self.addr in self.esp32s2_rom_mem_range:
                result = True
        return result
...
class SymbolList(object):
    ...
    """
    list_pattern_dict = {   'OBJ_NAME': (r'([_a-zA-Z0-9]+\.c\.obj)\:\s+ file format elf32-xtensa-[ble]{2}'),
                            'OBJ_SYM_SKIP1': (r'([a-zA-Z0-9]+)\sl\s+d\s+([\*\.a-zA-Z0-9_]+)\s+([a-zA-Z0-9]+)\s(\.xtensa\.info.*)$'),
                            'OBJ_SYM_SKIP2': (r'([a-zA-Z0-9]+)\s([g\s]{1,2})\s+([F|\s]{1})\s+([\*\.a-zA-Z0-9]+)\s+([a-zA-Z0-9]+)\s(__assert_func.*)$'),
                            'OBJ_SYM_NORMAL': (r'([a-zA-Z0-9]+)\s([w|g|l\s]{1,2})\s+([O|F|d|\s]{1})\s+([\*\.a-zA-Z0-9_]+)\s+([a-zA-Z0-9]+)\s([\.]*.+\.)*(\w+)$')
                        }
                        # Skip these patterns:
                        #00000000 l    d  .xtensa.info	00000000 .xtensa.info
                        #00000000         *UND*	00000000 __assert_func
...
def test_check_wake_stub(env, extra_data):
    ...
    """
    try:
        dut1 = env.get_dut("test_wake_stub", "tools/test_apps/system/wake_stub", dut_class=ttfw_idf.ESP32DUT)
                ...
                for bin_symbol in binary_symbols:
                    if sys_symbol.name == bin_symbol.name:
                        if bin_symbol.check_symb_loc(target):
                            ...
                            raise ObjDumpError("Incorrect symbol placement %s" % sys_symbol.name)
        ...
        dut1.start_app()
        dut1.expect("I WAKE_STUB: Enter count: 4")
        dut1.expect("Wake stub ext0 text is passed.")
        dut1.expect("Setup wake stub timer test.")
        dut1.expect("I WAKE_STUB: Enter count: 4")
        dut1.expect("Test wake stub wakeup from timer is passed.")        
